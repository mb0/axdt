 ...
 *     
 *******************************************************************************/
// copied from org.eclipse.ui.preferences
...
public class ScopedPreferenceStore extends EventManager implements
	 ...
	 */
	public ScopedPreferenceStore(IScopeContext context, IScopeContext[] scopes, String qualifier,
			String defaultQualifierPath) {
		this(context, scopes, qualifier);
	 ...
	 *            the scope to store to
	 * @param scopes
	 * 			  the scope to use as default
	 * @param qualifier
	 *            the qualifier used to look up the preference node
	 */
	public ScopedPreferenceStore(IScopeContext context, IScopeContext[] scopes, String qualifier) {
		storeContext = context;
		setSearchContexts(scopes);
	...
	public IEclipsePreferences[] getDefaultPreferenceNodes() {
		if (searchContexts == null) {
			return new IEclipsePreferences[] { getDefaultPreferences() };
		}
		// otherwise the user specified a search order so return the appropriate
		// nodes based on it
		int length = searchContexts.length;
		IEclipsePreferences[] preferences = new IEclipsePreferences[length];
		for (int i = 1; i < searchContexts.length; i++) {
			preferences[i-1] = searchContexts[i].getNode(nodeQualifier);
		}
		preferences[length - 1] = getDefaultPreferences();
		return preferences;
	}
	...
	public void setSearchContexts(IScopeContext[] scopes) {
		...
		for (int i = 0; i < scopes.length; i++) {
			if (scopes[i].equals(defaultContext)) {
				Assert.isTrue(false, "error adding default");
			}
		}
	}
	...
	public boolean getDefaultBoolean(String name) {
		String value = internalGetDefault(name);
		return value == null ? BOOLEAN_DEFAULT_DEFAULT : Boolean.valueOf(value)
				.booleanValue();
	}
	...
	private String internalGetDefault(String key) {
		return Platform.getPreferencesService().get(key, null,
				getDefaultPreferenceNodes());
	}
	...
	public void setToDefault(String name) {
		...
		try {
			...
			if (oldValue == null && defaultValue != null || !oldValue.equals(defaultValue)){
				dirty = true;
